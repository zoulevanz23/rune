  ...
  return (
    <div style={{ maxWidth: 1320, margin: '0 auto', display:'flex', flexDirection:'column', gap: '2.8rem', paddingBottom:'1.2rem' }}>
      ...
      <div style={{ minHeight:'68vh', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', textAlign:'center', padding:'1rem 1rem 0.5rem', position:'relative' }}>
        ...
        <div style={{ position:'absolute', top:8, left:0, display:'flex', gap:6, fontFamily:"'IBM Plex Mono', monospace", fontSize:'0.52rem', letterSpacing:'0.08em', color:'var(--ink-soft)' }}>
        ...
        <div style={{ position:'absolute', top:8, right:0, fontFamily:"'IBM Plex Mono', monospace", fontSize:'0.52rem', letterSpacing:'0.08em', color:'var(--ink-soft)', border:'1px solid var(--grid-line)', background:'var(--surface)', padding:'3px 7px' }}>GRID 28 · SCALE 1:1 · REV 02</div>
        ...
        <div style={{ fontFamily:"'IBM Plex Mono', monospace", fontSize:'0.62rem', letterSpacing:'0.18em', color:'var(--ink-soft)', marginBottom:'1.1rem', border:'1px solid var(--grid-line)', padding:'5px 12px', background:'var(--surface)', display:'inline-block' }}>ROUGH IDEA → STRUCTURED PLAN</div>
        ...
        <div style={{ display:'flex', gap:14, alignItems:'center', flexWrap:'wrap', justifyContent:'center' }}>
          ...
          <a href="#how" onClick={e=>{e.preventDefault(); document.getElementById('how')?.scrollIntoView({behavior:'smooth'})}} style={{ fontFamily:"'IBM Plex Mono', monospace", fontSize:'0.7rem', color:'var(--ink-soft)', textDecoration:'underline', textUnderlineOffset:4 }}>See how it works</a>
        </div>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:0, width: 280, height:1, background:'var(--grid-line)', position:'relative' }}>
            {[0,25,50,75,100].map(t=>(
              <div key={t} style={{ position:'absolute', left:`${t}%`, top:-4, display:'flex', flexDirection:'column', alignItems:'center', transform:'translateX(-50%)' }}>
                ...
                <span style={{ fontFamily:"'IBM Plex Mono', monospace", fontSize:'0.5rem', color:'var(--ink-soft)', marginTop:2 }}>{t}</span>
              </div>
            ))}
          </div>
      ...
      <div id="how" ref={r1} style={{ scrollMarginTop: 24 }}>
        <div style={{ display:'flex', alignItems:'baseline', justifyContent:'space-between', marginBottom:'1.2rem', flexWrap:'wrap', gap:12 }}>
          ...
          <div style={{ fontFamily:"'IBM Plex Mono', monospace", fontSize:'0.6rem', letterSpacing:'0.1em', color:'var(--ink-soft)' }}>THREE STEPS — NO TEMPLATES</div>
        </div>
      ...
      <div id="methods" ref={r2} style={{ scrollMarginTop: 24 }}>
        <div style={{ fontFamily:"'IBM Plex Mono', monospace", fontSize:'0.6rem', letterSpacing:'0.1em', color:'var(--ink-soft)', marginBottom:'0.8rem' }}>METHODOLOGIES — ONE INSTRUMENT, TWO MODES</div>
            ...
            <div style={{ display:'flex', gap:7 }}>
              {[1,2,3].map(n=>(
                <div key={n} style={{ flex:1, border:'1px solid var(--grid-line)', background:'#F4EFE2', padding:'0.6rem', clipPath:'polygon(0 0, calc(100% - 6px) 0, 100% 6px, 100% 100%, 0 100%)' }}>
                  ...
                  <div style={{ fontFamily:"'IBM Plex Mono', monospace", fontSize:'0.58rem', color:'var(--ink)' }}>SPRINT {String(n).padStart(2,'0')}</div>
            ...
            <div style={{ display:'flex', gap:7 }}>
              {['Backlog','In Progress','Review','Done'].map((col,idx)=>(
                <div key={col} style={{ flex:1, border:'1px solid var(--grid-line)', background: idx%2? 'var(--surface)' : '#F4EFE2', padding:'0.55rem', clipPath:'polygon(0 0, calc(100% - 6px) 0, 100% 6px, 100% 100%, 0 100%)' }}>
                  <div style={{ fontFamily:"'IBM Plex Mono', monospace", fontSize:'0.58rem', color:'var(--ink)' }}>{col.toUpperCase()}</div>
                  {idx===1 && <div style={{ fontFamily:"'IBM Plex Mono', monospace", fontSize:'0.52rem', color:'#fff', background:'var(--coral)', border:'1px solid var(--coral)', display:'inline-block', padding:'1px 3px', marginTop:5 }}>WIP 3</div>}
      ...
      <div id="caps" ref={r3} style={{ scrollMarginTop: 24 }}>
        <div style={{ fontFamily:"'IBM Plex Mono', monospace", fontSize:'0.6rem', letterSpacing:'0.1em', color:'var(--ink-soft)', marginBottom:'0.8rem' }}>CAPABILITIES — SPEC SHEET</div>
        <div style={{ border:'1px solid var(--grid-line)', background:'var(--paper)', clipPath:'polygon(0 0, calc(100% - 10px) 0, 100% 10px, 100% 100%, 0 100%)' }}>
          {[
            ...
            <div key={r.code} style={{ display:'grid', gridTemplateColumns:'56px 130px 1fr', gap:14, alignItems:'center', padding:'0.95rem 1.1rem', borderBottom:'1px solid var(--grid-line)', borderLeft:`3px solid ${r.c}` }}>
              <span style={{ fontFamily:"'IBM Plex Mono', monospace", fontSize:'0.7rem', color:'var(--ink)' }}>{r.code}</span>
      ...
      <div ref={r4} style={{ textAlign:'center', padding:'1.2rem 1rem 0.2rem' }}>
        ...
        <div style={{ fontSize:'0.88rem', color:'var(--ink-soft)', marginBottom:'1.2rem' }}>No sign up. Your plan stays in your browser. Start drafting in the spec sheet.</div>
      ...
      <div style={{ borderTop:'1px solid var(--grid-line)', padding:'0.9rem 0', display:'flex', justifyContent:'space-between', alignItems:'center', flexWrap:'wrap', gap:10, background:'var(--canvas)' }}>
        <div style={{ display:'flex', alignItems:'center', gap:10, flexWrap:'wrap' }}>
          <span style={{ fontFamily:"'IBM Plex Mono', monospace", fontSize:'0.62rem', color:'var(--ink-soft)' }}>© {new Date().getFullYear()} Rune — instrument-grade agile drafting</span>
          <span style={{ fontFamily:"'IBM Plex Mono', monospace", fontSize:'0.62rem', color:'var(--ink-soft)' }}>· Developed by Josh Ivan Sartin</span>
        </div>
        <div style={{ display:'flex', alignItems:'center', gap:15 }}>
          <a href="https://github.com/zoulevanz23" target="_blank" rel="noopener noreferrer" style={{ display:'flex', alignItems:'center', gap:6, textDecoration:'none', color:'var(--ink-soft)', fontFamily:"'IBM Plex Mono', monospace", fontSize:'0.62rem' }}>
